        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-16">
          <div className="space-y-4">
            <p className="text-[10px] font-bold uppercase text-brand flex items-center gap-2">
               <LinkIcon size={12} /> Resource Graph
            </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {links.map((link, idx) => (
            <Link 
              key={idx} 
              href={link.url}
              className="group p-8 glass border border-stroke rounded-[2rem] hover:border-brand/40 transition-all duration-500 shadow-sm hover:shadow-premium bg-white relative overflow-hidden flex flex-col justify-between min-h-[160px]"
            >
              ...
              <div className="space-y-4 relative z-10">
                <span className="text-[8px] font-bold uppercase px-2 py-1 bg-surface border border-stroke rounded-md text-text-muted group-hover:text-brand group-hover:border-brand/30 transition-colors">
                  {link.relationship}
                </span>
              ...
              <div className="flex items-center gap-2 text-[10px] font-bold uppercase text-text-muted group-hover:text-brand transition-colors pt-4 mt-4 border-t border-stroke/50 relative z-10">
                 Explore Terminal <ArrowRight size={14} className="group-hover:translate-x-1 transition-transform" />
              </div>
            </Link>
          ))}
        </div>
