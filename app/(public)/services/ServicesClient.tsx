  ...
  return (
    <div className="min-h-screen bg-white text-text-primary">
      <main className="grain">
        ...
        <section className="relative pt-20 md:pt-32 pb-16 md:pb-24 border-b border-stroke overflow-hidden">
          ...
          <div className="container-max relative z-10 text-center space-y-8 md:space-y-12">
            ...
            >
              ...
              <p className="text-text-secondary max-w-3xl mx-auto text-lg md:text-2xl font-medium leading-relaxed antialiased px-4 md:px-0">
                {hero.subtext}
              </p>
            </motion.div>
          </div>
        ...
        <section className="section-padding">
          <div className="container-max space-y-16 md:space-y-32">

            <div id="marketing" className="scroll-mt-32 space-y-8 md:space-y-16">
               <div className="space-y-4 md:space-y-6">
                  <p className="text-[11px] font-bold uppercase text-brand tracking-[0.3em]">{pillar1.badge}</p>
                  <h2 className="text-2xl md:text-5xl font-bold uppercase tracking-tight leading-tight antialiased">{pillar1.heading} <br /> <span className="text-brand">{pillar1.headingAccent}</span></h2>
               </div>
            ...
            <div id="design" className="scroll-mt-32 space-y-8 md:space-y-16">
               <div className="space-y-4 md:space-y-6 text-left md:text-right">
                  ...
                  <h2 className="text-2xl md:text-5xl font-bold uppercase tracking-tight leading-tight antialiased">{pillar2.heading} <br /> <span className="text-brand">{pillar2.headingAccent}</span></h2>
               </div>
            ...
            <div id="development" className="scroll-mt-32 space-y-8 md:space-y-16">
               <div className="space-y-4 md:space-y-6">
                  <p className="text-[11px] font-bold uppercase text-brand tracking-[0.3em]">{pillar3.badge}</p>
                  <h2 className="text-2xl md:text-5xl font-bold uppercase tracking-tight leading-tight antialiased">{pillar3.heading} <br /> <span className="text-brand">{pillar3.headingAccent}</span></h2>
               </div>
        ...
        <section className="section-padding bg-surface/30 border-y border-stroke relative overflow-hidden">
           <div className="container-max relative z-10">
              <div className="text-center mb-12 md:mb-24 space-y-4 md:space-y-6">
                 ...
                 <h2 className="text-2xl md:text-5xl font-bold uppercase tracking-tight antialiased">{techStack.heading} <span className="text-brand">{techStack.headingAccent}</span></h2>
              </div>
        ...
        <section className="py-16 md:py-48 bg-white relative overflow-hidden grain border-t border-stroke text-center">
          <div className="container-max space-y-8 md:space-y-16">
            <h2 className="text-2xl md:text-5xl font-bold uppercase tracking-tight antialiased">Ready to <span className="text-brand">Launch?</span></h2>
...
const ServiceSummaryCard = React.memo(({ service }: { service: any }) => {
  ...
  return (
    <motion.div 
      ...
      className="glass group p-6 md:p-10 flex flex-col justify-between hover:border-brand/40 transition-all duration-500 rounded-3xl md:rounded-[3rem] shadow-sm hover:shadow-premium h-full border border-stroke bg-white relative overflow-hidden will-change-gpu"
    >
       <div className="space-y-6 md:space-y-8 relative z-10">
          ...
          <div className="space-y-3 md:space-y-4">
             <h3 className="text-2xl md:text-3xl font-bold uppercase tracking-tight leading-none group-hover:text-brand transition-colors antialiased">
                {service.title}
             </h3>
             <p className="text-text-muted text-base md:text-lg font-medium leading-relaxed antialiased line-clamp-4">
                {service.description}
             </p>
          </div>
       </div>
       <div className="pt-6 mt-6 md:pt-8 md:mt-10 border-t border-stroke relative z-10">
...
const TechCard = React.memo(({ icon: Icon, label }: any) => {
  return (
    <motion.div 
      ...
      viewport={{ once: true, margin: "0px" }}
      transition={{ duration: 0.4 }}
      className="glass border border-stroke rounded-2xl md:rounded-[2rem] flex flex-col items-center justify-center gap-4 md:gap-6 hover:border-brand transition-all duration-700 shadow-premium group cursor-default p-5 md:p-8 bg-white/50 backdrop-blur-sm will-change-gpu"
    >
...
const StackItem = React.memo(({ title, desc }: any) => {
  return (
    <motion.div 
      ...
      viewport={{ once: true, margin: "0px" }}
      transition={{ duration: 0.4 }}
      className="flex flex-col md:flex-row gap-6 md:gap-8 group bg-surface border border-stroke p-6 md:p-10 rounded-3xl md:rounded-[2.5rem] hover:border-brand transition-all items-center md:items-start text-center md:text-left will-change-gpu"
    >
      ...
      <div>
        <h4 className="text-xl md:text-2xl font-bold mb-2 md:mb-3 uppercase tracking-tight leading-none antialiased group-hover:text-brand transition-colors">{title}</h4>
        <p className="text-text-muted text-sm md:text-lg font-medium leading-relaxed antialiased">{desc}</p>
      </div>
    </motion.div>
  );
});
