    ...
    <div className="space-y-12">
      ...
      <div className="flex justify-between items-end">
         <div className="space-y-4">
            ...
            <p className="text-text-muted text-sm font-medium uppercase tracking-widest">Status: Online | All Systems Active</p>
         </div>
         <div className="flex gap-4">
            <button className="btn-outline h-12 px-6 text-[10px] font-bold uppercase tracking-widest shadow-sm">Export Data</button>
            <button className="btn-primary h-12 px-6 text-[10px] font-bold uppercase tracking-widest shadow-xl shadow-brand/10">Manage Clients</button>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
         ...
         <div className="lg:col-span-8 bg-white rounded-[2rem] border border-stroke p-10 space-y-8">
            <div className="flex justify-between items-center">
               ...
               <Link href="/admin/leads" className="text-[10px] font-bold uppercase tracking-widest text-brand hover:underline">View All Leads</Link>
            </div>
            ...
            <div className="space-y-4">
               {recentLeads.map((lead, i) => (
                 <div key={lead.id} className="flex items-center justify-between p-6 rounded-2xl hover:bg-surface transition-colors cursor-pointer group">
                    ...
                    <div className="flex items-center gap-12">
                       <span className="hidden md:block text-[10px] font-bold uppercase tracking-widest text-text-muted max-w-[150px] truncate">{lead.email}</span>
               ...
               {recentLeads.length === 0 && (
                 <p className="text-center py-10 text-text-muted font-bold uppercase tracking-widest text-[10px]">No recent leads found.</p>
               )}
            </div>
         </div>
