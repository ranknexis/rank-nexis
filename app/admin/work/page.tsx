  ...
  return (
    <div className="space-y-10">
      <div className="flex justify-between items-end">
        ...
        <Link href="/admin/work/new" className="btn-primary h-14 px-8 text-[10px] font-bold uppercase flex items-center gap-3 shadow-xl shadow-brand/10">
      ...
      <div className="bg-white rounded-[2rem] border border-stroke overflow-hidden shadow-sm">
        <div className="p-8 border-b border-stroke flex justify-between items-center bg-surface/30">
          <div className="relative w-full md:w-96">
            ...
            <input type="text" placeholder="Filter case studies..." className="w-full h-12 bg-white border border-stroke rounded-xl pl-16 pr-6 text-[10px] font-bold uppercase focus:outline-none focus:border-brand transition-all" />
          </div>
          <p className="text-[9px] font-bold uppercase text-text-muted">Total records: <span className="text-brand">{studies.length}</span></p>
        </div>
                ...
                <tr key={study.id} className="hover:bg-surface/30 transition-colors group">
                  ...
                  <td className="px-10 py-8 text-right">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <Link href={`/admin/work/${study.id}`} className="p-3 bg-white border border-stroke hover:bg-brand hover:text-white rounded-lg transition-all text-text-muted shadow-sm"><Edit2 size={16} /></Link>
              ...
              {studies.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-10 py-24 text-center">
                    <p className="text-text-muted font-bold uppercase text-[10px]">No case studies found in the database archive.</p>
                  </td>
                </tr>
              )}
            </tbody>
